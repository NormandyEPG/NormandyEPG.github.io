#!/bin/bash
#### Descripción: Instalador NormandyEPG para TVH add-on AlexELEC
...
CARPETA_TVH="/storage/.config/tvheadend"
...
██║ ╚████║╚██████╔╝██║  ██║██║ ╚═╝ ██║██║  ██║██║ ╚████║██████╔╝   ██║   ███████╗██║     ╚██████╔╝
╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝    ╚═╝   ╚══════╝╚═╝      ╚═════╝ 
Monkey Brothers Team | Instalador NormandyEPG universal - AlexELEC
...
##Borramos viejos grabbers de Normandy
			rm -f /storage/.config/tvheadend/bin/tv_grab_NormandyEPG
			rm -f /storage/.config/tvheadend/bin/tv_grab_NormandyEPG2
			rm -f /storage/.config/tvheadend/bin/tv_grab_NormandyEPGmv
			rm -f /storage/.config/tvheadend/bin/tv_grab_NormandyEPGnm
...
##Copiamos archivos de la carpeta temporal a las rutas definitivas
	...
	echo
		...
		cd /storage/.kodi/
			...
			cp -r $CARPETA_NORMANDYTEMP/bin/* $CARPETA_TVH/bin
...
##Reiniciamos el servicio de TVH y avisamos del grabber
	echo	
